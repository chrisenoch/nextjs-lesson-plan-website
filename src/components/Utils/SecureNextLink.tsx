 ...
 * because it seems that sometimes Next.js solves bugs in one version and then they reappear in later versions. To ensure a stable application, we solve the issue ourselves.
 ...
 *
 * NextLink also sometimes skips rendering server components due to its soft navigation. This means that we cannot reliably protect
 ...
 *
 * Limitations: Does not suppport segments (e.g. foo.com/home#title), as segments are not sent to the server. Also a developer may use NextLink by mistake.
 * However, this is the only approach that ensures middleware is activated every time.
 */
